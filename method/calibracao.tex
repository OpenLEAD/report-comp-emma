...
Como especificado no capítulo \ref{cap::detail}, o sensor a ser adquirido pelo
projeto é o Faro Focus X330.
...
estruturada e controlada, na etapa de construção do sistema, assegurando assim
uma calibração entre o posicionamento dos marcadores e o manipulador.


\begin{figure}[H]
   \centering
   \includegraphics[width=0.5\columnwidth]{method/figs/calibracao/sphere_rec}
   \caption{Exemplo de esfera utilizada para reconhecimento. Fonte:
   \url{http://shop.talwin.net/}.}
...
\begin{figure}[H]
   \centering
   \includegraphics[width=0.5\columnwidth]{method/figs/calibracao/checkerboard_rec}
   \caption{Exemplo de padrão de xadrez utilizado para reconhecimento.\\ Fonte:
   \url{http://stereomorph.blogspot.com.br}.}
...
à Usina Hidrelética Jirau para o teste de viabilidade técnica desse sensor.
...
dos sensores também estão disponí\-veis para ajuste, assim como o seu respectivo
nível de ruído.
...
existe a necessidade do desenvolvimento de um método de amostragem que favoreça
as regiões com maior número de \textit{features}, ou características, e durante
o alinhamento, penalize o alinhamento entre regiões planas. Outro quesito em aberto é o aprimoramento na medição do erro de estimação do posicionamento da
...
redu\-ção da complexidade computacional, porém desenvolvimento da calibração do
